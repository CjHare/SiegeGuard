...
export async function mvpDemoPolicy(): Promise<MvpDemoPolicy> {
  return await dns
    .lookup(DomainName.of(MvpDemoPolicy.name))
    .then(
      (address: EthereumAddress) => new MvpDemoPolicy(web3, sender, address)
    )
}
